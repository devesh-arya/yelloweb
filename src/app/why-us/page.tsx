...
export default function WhyUsPage() {
  return <>
    <section className="px-8 lg:px-16 pt-40 pb-16 bg-canvas-2 border-b border-border relative overflow-hidden">
      ...
      <FadeUp delay={0.1}><h1 className="font-serif text-[clamp(3rem,6vw,5rem)] font-semibold text-teal max-w-[700px] leading-tight">We build differently — <TextFillAnimation>on purpose</TextFillAnimation> </h1></FadeUp>
      <DrawLine delay={0.3} className="mt-6 mb-4 max-w-xs" color="bg-y-gradient" />
      <FadeUp delay={0.2} className="max-w-[520px] text-lg text-ink-body leading-relaxed">The market is full of studios who can write code. What sets us apart is the depth with which we engage with your goals.</FadeUp>
    </section>
    <WhyUsGrid />
      ...
      <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        {STATS.map(s => (
          <StaggerItem key={s.label} className="text-center">
            ...
            <div className="text-xs tracking-[0.15em] uppercase text-ink-body">{s.label}</div>
          </StaggerItem>
        ))}
      </StaggerContainer>
    </section>
