...
export default function ServicesPage() {
  return <>
    <section className="px-8 lg:px-16 pt-40 pb-16 bg-canvas-2 border-b border-border relative overflow-hidden">
      ...
      <FadeUp delay={0.2} className="max-w-[520px] text-lg text-ink/55 leading-relaxed">Five core disciplines. One integrated studio.</FadeUp>
    </section>
        ...
        {SERVICES.map(s => (
          <StaggerItem key={s.title}>
            <HoverCard className="h-full bg-white border border-border p-10 group">
              <div className="font-serif text-5xl font-light text-teal/20 mb-6 group-hover:text-teal/40 transition-colors">{s.number}</div>
