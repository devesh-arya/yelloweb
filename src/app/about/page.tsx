...
export default function AboutPage() {
  return <>
    <section className="px-8 lg:px-16 pt-40 pb-16 bg-canvas-2 border-b border-border relative overflow-hidden">
      ...
      <FadeUp delay={0.1}><h1 className="font-serif text-[clamp(3rem,6vw,5rem)] font-semibold text-teal max-w-[700px] leading-tight">Built on <TextFillAnimation>listening</TextFillAnimation>, driven by craft</h1></FadeUp>
      <DrawLine delay={0.3} className="mt-6 mb-4 max-w-xs" color="bg-y-gradient" />
      <FadeUp delay={0.2} className="max-w-[520px] text-lg text-ink-body leading-relaxed">We're a full-stack web development studio that believes the best digital experiences begin long before a line of code is written.</FadeUp>
    </section>
    <section className="px-8 lg:px-16 py-32 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 bg-white">
      <div>
        ...
        <FadeUp delay={0.1} as="h2" className="font-serif text-[clamp(2rem,3.5vw,3rem)] font-semibold text-teal mb-6">"Every project begins with <TextFillAnimation>listening</TextFillAnimation>"</FadeUp>
        <FadeUp delay={0.2} className=" mb-6">We take the time to understand your goals, your audience, and the values that shape your organization. From there, we collaborate closely to design and develop a website that authentically reflects who you are.</FadeUp>
        <FadeUp delay={0.25} className="">We don't believe in templates. Every project is built from first principles.</FadeUp>
      </div>
      <div>
        <FadeUp className="section-tag mb-4">Our Foundations</FadeUp>
        <FadeUp delay={0.1} as="h2" className="font-serif text-[clamp(2rem,3.5vw,3rem)] font-semibold text-teal mb-6">What we're <TextFillAnimation>built</TextFillAnimation> on</FadeUp>
        <StaggerContainer>
